...
def run_hunting_game_chap3(screen,borders,huntworld,time,night,framelists,wolfGraphics,training=False): # All in function, so can run from other screen.
    ...
    while True: # End the loop by returning something
        ...
        if doesCol:
            if isinstance(doesCol,Animal): # Should always be true.
                if isinstance(doesCol,Rabbit):  # If the player catches a rabbit, instant kill.
                    ...
                    # doesCol.health -= 45
                    # doesCol.speed = doesCol.health*doesCol.maxspeed//200
                    # if doesCol.dead:
                    huntworld.animals.remove(doesCol)
                    huntworld.interactives.remove(doesCol)
                    huntworld.objectsofheight.remove(doesCol)
                    ...
                    doesCol.speed = doesCol.health*doesCol.maxspeed//50
                    doesCol.health -= 50
                    huntworld.animals.remove(doesCol)
                    huntworld.interactives.remove(doesCol)
                    huntworld.objectsofheight.remove(doesCol)
                # elif isinstance(doesCol,Wolf): # For deer - slow kill
        ...
        if preyalldead:
            ...
            if training:
                dialog.akela(screen,f"{len(huntworld.animals)-2} got away. We'll catch some next time.")
            for scavengeable in packkills:
                health += scavengeable.maxstrength / (len(huntworld.animals)+1)
            if health > 100:
                health = 100
