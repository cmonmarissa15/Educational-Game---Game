...
# This script contains three functions:  1, getWorldGraphics_chap3, which loads all the
...
def getWorldGraphics_chap3(window_height,worldx=0,worldy=0,bgname="Map_Background.png"):
    ...
    def getPrintGraphics(height): # Height is the height of the world, by which prints are scaled.
        ...
        for animal in ['wolf']: # List neither predator nor prey here - raccoons, foxes
            printImage = pygame.image.load(os.path.join('Assets',animal+"_print.png"))
            printImageSmall = pygame.transform.scale(printImage,(int(height/15),int(height/15)))
            printGraphics[animal] = printImage
            printGraphicsSmall[animal] = printImageSmall
    ...
    def getAnimalGraphics(animalTypes,height):
        ...
        for animal in animalTypes: # Iterate through the keys of dictionary animalTypes.
            ...
            for frame in range(1,5):
                ...
                elif animal == 'wolf':  # This does cause pixely images in the dialog, though.
                    framer = pygame.transform.scale(framer,(int(height*framer.get_width()/(6*framer.get_height())),int(height/6)))
...
def generateWorld_3(worldx,worldy,window_width,window_height,background, nightbackground, streamAppearancesByAim, streamNightAppearancesByAim, streamDimensionsByAim, streamCurveCoefficients, treeGraphics, treeNightGraphics, treeGreenness, rockGraphics, rockNightGraphics, decorGraphics, decorNightGraphics, decorDynamics, printGraphicsSmall, miscellaneousGraphics, miscellaneousNightGraphics, Den_call):
    ...
    def leavePrints(worldx,worldy,window_width,window_height,printGraphicsSmall,streams):
        ...
        prints = []
        for i in range(6,10):
            prints.append(stampPrint(worldx,worldy,window_width,window_height,'wolf',streams,printGraphicsSmall)) #'wolf' ensure that we select wolf from the dictionary
...
def makeHuntWorld(oldworld,centerx,centery,window_width,window_height,prey,animalGraphics,night,pack=False,wolfGraphics={ },maincharname='',packmemcount=0):
    ...
    def birthPreyAnimal(borders,window_width,window_height,animal,obstacles,animalGraphics):
        ...
        preycounts = {'rabbit':random.randint(3,10),'deer':random.randint(1,3),'bison':1, 'wolf': 6}
        ...
        while len(preylist) < count:
            ...
            if posok(x,y,obstacles):
                ...
                # elif animal == 'wolf': # Else there would be an uglier if-else tree in 'move()'.
                    # preylist.append(Wolf(x,y,appearance[0][0].get_height(),appearance[0][0].get_width(),appearance))
                else:
