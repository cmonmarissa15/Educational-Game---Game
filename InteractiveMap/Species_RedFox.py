...
import pygame
import SpeciesMenu
import RedFox.identification     as Identification
import RedFox.traits             as Traits
import RedFox.diet               as Diet
import RedFox.habitat            as Habitat
import RedFox.threats            as Threats
import RedFox.ecosystem          as Ecosystem
import RedFox.conservation       as Conservation
import RedFox.wolfpark           as WolfPark
import RedFox.protect            as Protect
    ...
    while True: 
        for ev in pygame.event.get(): 
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN: 
                if  20 <= mouse[0] <= width / 5 + 20 and height/4 <= mouse[1] <= height/2 + 50:                     Identification.redfox_id()
                if  width/5 + 20 <= mouse[0] <= 2*width/5 + 10 and height/4 <= mouse[1] <= height/2 + 50:           Traits.redfox_traits()
                if  2*width/5+10 <= mouse[0] <= 3*width/5 and height/4 <= mouse[1] <= height/2 + 50:                Diet.redfox_diet()
                if  3*width/5 <= mouse[0] <= 4*width/5 - 10 and height/4 <= mouse[1] <= height/2 + 50:              Habitat.redfox_habitat()
                if  4*width/5 -10 <= mouse[0] <= width - 20 and height/4 <= mouse[1] <= height/2 + 50:              Threats.redfox_threats()
                if  20 <= mouse[0] <= width / 5 + 20 and height/2 + 50 <= mouse[1] <= 3*height/4 + 100:             Ecosystem.redfox_ecosystem()
                if  width/5 + 20 <= mouse[0] <= 2*width/5 + 10 and height/2 + 50 <= mouse[1] <= 3*height/4 + 100:   Conservation.redfox_conservation()
                if  2*width/5+10 <= mouse[0] <= 3*width/5 and height/2 + 50 <= mouse[1] <= 3*height/4 + 100:        WolfPark.redfox_wolfpark()
                if  3*width/5 <= mouse[0] <= 4*width/5 - 10 and height/2 + 50 <= mouse[1] <= 3*height/4 + 100:      Protect.redfox_protect()
        ...
        # Sets the Red Fox Species Page as the BACKGROUND 
        RedFox_Image = pygame.image.load('./Assets/RedFox/SpeciesPage-RedFox.png')
        screen.blit(RedFox_Image, (0, 0))
        ...
        # if mouse is hovered over eighth button (Red Foxes at Wolf Park) it changes to lighter shade 
        ...
        # if mouse is hovered over ninth button (How can people protect Red Foxes?) it changes to lighter shade 
