    ...
    while True: 
        for ev in pygame.event.get(): 
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN: 
                ...
                if 20 <= mouse[0] <= 750 and 20 <= mouse[1] <= 60: Identification.redfox_id() 
                
                #if the mouse is clicked on the second button (Unique traits, behaviors, and abilities)
                if width/4 <= mouse[0] <= width/4 + 380 and height/4 <= mouse[1] <= height/4 + 40:              Traits.redfox_traits()
                
                #if the mouse is clicked on the third button (Diet)
                if width/2 <= mouse[0] <= width/2+150 and height/6 - 100 <= mouse[1] <= height/6 - 60:          Diet.redfox_diet()
                
                #if the mouse is clicked on the fourth button (Habitat)
                if 3*width/4 <= mouse[0] <= 3*width/4 + 150 and height/4 - 40 <= mouse[1] <= height/4:          Habitat.redfox_habitat()
                
                #if the mouse is clicked on the fifth button (Threats)
                if width/8 <= mouse[0] <= width/8 + 150 and height/2 + 50 <= mouse[1] <= height/2 + 90:         Threats.redfox_threats()
                
                #if the mouse is clicked on the sixth button (Role in the Ecosystem)
                if width/4-100 <= mouse[0] <= width/4+150 and 3*height/4 <= mouse[1] <= 3*height/4 + 40:        Ecosystem.redfox_ecosystem()
                
                #if the mouse is clicked on the seventh button (Conservation Status)
                if width/2-70 <= mouse[0] <= width/2+180 and 3*height/4-140 <= mouse[1] <= 3*height/4 -100:     Conservation.redfox_conservation()
                
                #if the mouse is clicked on the eigth button (Bison and Wolf Park)
                if 3*width/4-70 <= mouse[0] <= 3*width/4+200 and height/2-40 <= mouse[1] <= height/2:           WolfPark.redfox_wolfpark()
                
                #if the mouse is clicked on the ninth button (How can people protect Bison?)
                if 3*width/4-50 <= mouse[0] <= 3*width/4 + 350 and 3*height/4 <= mouse[1] <= 3*height/4 + 40:   Protect.redfox_protect()
