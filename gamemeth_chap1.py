...
def getworldgraphics_chap3(height): # Where height is the height of the world, used to scale images.
    ...
    wolfImage = pygame.image.load(os.path.join('Assets_poc',"indicator_paw.png"))
    wolfImage = pygame.transform.scale(wolfImage,(int(height/15),int(height/10)))
    printImages = [[bisonImage],[rabbitImage],[deerImage],[wolfImage]]
...
# of the player.
def drawscreen(screen,height,width,background,timelapsed,playerx,playery,allImageLists,allNightImages,allTypeOffsetRanges,allLocations,allYs,allTypes,allOffsets,framelists,currentmode,currentframe,health,NPC_health):
    ...
    NPC_healthBarRect = pygame.Rect(int(5*width/56 - height/24),int(11*height/12),int(NPC_health),int(height/24))
...
# Retrieve graphics.
printImages, printTypeCollideBoxes, obstacleImages, obstacleNightImages, obstacleTypeCollideBoxes, passableImages, passableNightImages = getworldgraphics_chap3(height)
# Generate world.
